...
export default function Home() {
  ...
  const [wrap, setWrap] = useState<string>(wrapOptions[0]);
  const [deliveryAddress, setDeliveryAddress] = useState("");
  const [contactPhone, setContactPhone] = useState("");
  const [messageText, setMessageText] = useState("");

  const categories = useMemo(() => {
    const set = new Set<string>(["Occasions", "Flower Type", "Price Range"]);
    return ["all", ...Array.from(set)];
  }, []);

  const filteredProducts = useMemo(() => {
    if (selectedCategory === "all") return products;
    if (selectedCategory === "Occasions") return products.filter((p) => p.categories.some((c) => c.startsWith("Occasion:")));
    if (selectedCategory === "Flower Type") return products.filter((p) => p.categories.some((c) => c.startsWith("FlowerType:")));
    if (selectedCategory === "Price Range") return products.filter((p) => p.categories.some((c) => c.startsWith("Price:")));
    return products;
  ...
  return (
    <main className="min-h-screen pb-24">
      <section className="relative overflow-hidden bg-pink-50 pb-12 pt-28">
