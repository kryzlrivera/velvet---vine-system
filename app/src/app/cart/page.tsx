...
function tryParseLatLng(input: string): { lat: number; lng: number } | null {
  const s = input.trim();
  if (!s) return null;
...
export default function CartPage() {
  ...
  return (
    <main className="min-h-screen pb-24 pt-24 bg-pink-50">
      <section className="mx-auto max-w-6xl px-6">
        <h1 className="mb-4 text-4xl font-bold text-pink-700">Shopping Cart</h1>
        {cartDetails.length === 0 ? (
          <p className="rounded-xl border border-pink-200 bg-white p-6 text-pink-600">Your cart is empty. Go back to <a href="/" className="font-semibold text-pink-700">shop</a>.</p>
        ) : (
                ...
                <div className="mt-4 space-y-3">
                  ...
                  <label className="block">
                    ...
                    <select
                      ...
                      onChange={(e) => setDeliveryOption(e.target.value as "Standard" | "Express" | "Same-day")}
                      className="mt-1 w-full rounded-lg border border-pink-300 px-3 py-2"
                    >
                  ...
                  <button
                    ...
                    onClick={() => {
                      placeOrder(
                        deliveryAddress,
                        contactPhone,
                        deliveryOption as "Standard" | "Express" | "Same-day",
                        paymentMethod,
                        deliveryLatLng,
                      );
